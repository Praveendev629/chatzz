...
const { createStatus, getStatuses, viewStatus, getStatusViewers, deleteStatus, cleanupExpiredStatuses } = require('../controllers/statusController');

router.get('/', protect, getStatuses);
router.post('/', protect, messageUpload.single('media'), createStatus);
router.post('/cleanup', protect, cleanupExpiredStatuses);
